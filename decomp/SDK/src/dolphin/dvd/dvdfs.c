...
s32
DVDReadPrio (DVDFileInfo* fileInfo, void* addr, s32 length, long offset, long prio)
{
    int              result;
                  ...
                  long         prio)
{
    ...
    DVDSeekAbsAsyncPrio (&fileInfo->cb,
                         (u32)(u8*)fileInfo->startAddr + offset,
                         cbForSeekAsync,
                         prio);
    return 1;
}
...
s32
DVDSeekPrio (DVDFileInfo* fileInfo, s32 offset, long prio)
{
    BOOL             result;
    DVDCommandBlock* block;
    s32              state;
