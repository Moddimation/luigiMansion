...
#define INSTR_PSQ_ST(psr, offset, rDest, w, gqr)                               ﾂ･
	(0xF0000000 | (psr << 21) | (rDest << 16) | (w << 15) | (gqr << 12)        ﾂ･
	 | offset)
#define INSTR_PSQ_L(psr, offset, rSrc, w, gqr)                                 ﾂ･
	(0xE0000000 | (psr << 21) | (rSrc << 16) | (w << 15) | (gqr << 12) | offset)
#define INSTR_STW(rSrc, offset, rDest)                                         ﾂ･
	(0x90000000 | (rSrc << 21) | (rDest << 16) | offset)
#define INSTR_LWZ(rDest, offset, rSrc)                                         ﾂ･
	(0x80000000 | (rDest << 21) | (rSrc << 16) | offset)
#define INSTR_STFD(fprSrc, offset, rDest)                                      ﾂ･
	(0xD8000000 | (fprSrc << 21) | (rDest << 16) | offset)
#define INSTR_LFD(fprDest, offset, rSrc)                                       ﾂ･
	(0xC8000000 | (fprDest << 21) | (rSrc << 16) | offset)
#define INSTR_MFSPR(rDest, spr)                                                ﾂ･
	(0x7C000000 | (rDest << 21) | ((spr & 0xFE0) << 6) | ((spr & 0x1F) << 16)  ﾂ･
	 | 0x2A6)
#define INSTR_MTSPR(spr, rSrc)                                                 ﾂ･
	(0x7C000000 | (rSrc << 21) | ((spr & 0xFE0) << 6) | ((spr & 0x1F) << 16)   ﾂ･
	 | 0x3A6)
...
static DSError TRKTargetEnableTrace(BOOL val)
{
	if (val) {
		...
		gTRKCPUState.Extended1.MSR = (gTRKCPUState.Extended1.MSR & 窶ｾ0x400);
	}
	return DS_NoError;
}
